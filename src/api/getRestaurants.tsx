...
import { checkRestaurantOpen } from '@/api/checkRestaurantOpen'
import { getPriceRangeByID } from '@/api/getPriceRangeByID'
  ...
  try {
    ...
    const newData = await Promise.all(
      (data.restaurants as Restaurant[]).map(async (restaurant) => {
        const priceRange = await getPriceRangeByID({
          id: restaurant.price_range_id
        })
        const open = await checkRestaurantOpen({ id: restaurant.id })
        return {
          ...restaurant,
